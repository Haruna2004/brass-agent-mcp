...
class BrassService {
  ...
  constructor() {
    ...
    this.api = axios_default.create({
      baseURL: BRASS_API_BASE_URL,
      timeout: 1e4
    });
  }
  async confirmAccount(bankCode, accountNumber, brassToken) {
    try {
      const options = {
        headers: {
      ...
      const response = await this.api.get(`${BRASS_API_PATHS.resolveName}?bank=${bankCode}&account_number=${accountNumber}`, options);
      return {
        success: true,
  ...
  async createPayment(payable, brassToken) {
    try {
      const response = await this.api.post(BRASS_API_PATHS.createPayment, payable, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${brassToken}`
        }
...
// src/tools/confirmAccount.ts
var singleConfirmAcctSchema = z.object({
...
var bulkConfirmAcctSchema = z.object({
  accountsToConfirm: z.array(singleConfirmAcctSchema).describe("List of accounts (bank code and number) to verify.")
});
var brassService = new BrassService;
var brassToken = process.env.BRASS_PA_TOKEN;
var confirmMultipleAccounts = async ({
  accountsToConfirm
}) => {
  console.log("Attempting to confirm accounts:", accountsToConfirm);
  const results = await Promise.allSettled(accountsToConfirm.map((account) => brassService.confirmAccount(account.bankCode, account.accountNumber, brassToken)));
  const processedResults = results.map((result, index) => {
    const inputAccount = accountsToConfirm[index];
    if (result.status === "fulfilled") {
      const apiResult = result.value;
      if (apiResult.success) {
        console.log(`Confirmation Success for ${inputAccount?.accountNumber}:`, apiResult.data);
        return {
          input: inputAccount,
          status: "success",
          data: apiResult.data
        ...
        console.log(`Confirmation Failed for ${inputAccount?.accountNumber}:`, apiResult.error);
        return {
          input: inputAccount,
          status: "error",
      ...
      console.error(`Confirmation System Error for ${inputAccount?.accountNumber}:`, result.reason);
      return {
        input: inputAccount,
        status: "error",
        error: {
          code: "TOOL_EXECUTION_ERROR",
          message: "Failed to communicate with the account confirmation service."
        }
      };
    }
  });
  return {
    content: [
      { type: "text", text: JSON.stringify(processedResults) }
    ]
  };
};
...
// src/tools/processPayment.ts
import { randomUUID } from "crypto";
var BrassPayableSchema = z.object({
...
var singlePaySchema = z.object({
  title: z.string().describe("A brief memo/description for the payment transaction."),
  amount: z.number().positive().describe("The payment amount in the base currency unit."),
  name: z.string().describe("The verified recipient name."),
  accountNumber: z.string().length(10).regex(/^\d{10}$/).describe("The 10-digit verified account number."),
  bankId: z.string().describe("The unique bank identifier (usually same as bank code)."),
...
async function processMultiplePayments({
  ...
  const results = await Promise.allSettled(paymentsToProcess.map(async (payment) => {
    const payable = {
    ...
    return brassService2.createPayment(payable, brassToken2);
  }));
  const processedResults = results.map((result, index) => {
    ...
    if (result.status === "fulfilled") {
      ...
      if (apiResult.success) {
        ...
        console.log(`Payment Failed for ${inputPayment?.accountNumber}:`, apiResult.message);
        return {
          input: inputPayment,
          status: "error",
          message: apiResult.message ?? "Payment processing failed."
        };
      }
    } else {
      console.error(`Payment System Error for ${inputPayment?.accountNumber}:`, result.reason);
      return {
        input: inputPayment,
        status: "error",
